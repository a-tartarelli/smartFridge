...
unsigned short int is_intero (char input[]) {
	int input_corretto = 0;                   /*variabile che quando verra'  posta a 1 determinera'  che l'input e' un intero, altrimenti se non e' un intero verra'  posta a 0*/
	...
	for( int i=0; i<strlen(input); i++){      /*ciclo che itera per ogni carattere della stringa in input*/
		...
		if( isdigit(input[i]) ){    /*controlla che tutti i caratteria siano cifre*/
			...
			return 0;             /*se c'e anche un solo carattere che non e' una cifra, verra'  restituito 0*/
		}

	}
	return input_corretto;    /*se tutti i caratteri sono cifre, verra'  restituito 1*/
}
...
/**
 * @pre min deve essere un decimale compreso tra 1,175494351 E – 38 e 3,402823466 E + 38
 * @pre max deve essere un decimale compreso tra 1,175494351 E – 38 e 3,402823466 E + 38
 * @pre numero deve essere un decimale compreso tra 1,175494351 E – 38 e 3,402823466 E + 38
 * @post il valore restituito deve esse 1 se numero e' compreso nell'intervallo, 0 altrimenti
 */
...
unsigned short int is_stringa_caratteri (char stringa[]) {
	...
	for (int i=0; i<strlen(stringa); i++){		 /*ciclo che itera per ogni carattere della stringa in input*/
		if (isalpha (stringa[i])) {				 /*controlla che tutti i caratteria siano lettere*/
			...
			return 0;				/*se c'e anche un solo carattere che e' una cifra o un simbolo, verra'  restituito 0*/
		}
	}
	return stringa_corretta;		/*se tutti i caratteri sono lettere, verra'  restituito 1*/
}
...
unsigned short int is_float (char input[]) {
	int input_corretto = 0;                   /*variabile che quando verra'  posta a 1 determinera'  che l'input e' un float, altrimenti se non e' un float verra'  posta a 0*/
...
data inserisci_input_data (char messaggio[], unsigned short int lunghezza_input) {
	char* input = (char*) calloc(LUNGHEZZA_NOME, sizeof(char));		/*allocazione dinamica della memoria per la stringa input*/
	data data_input;			/*variabile di tipo data in cui verra'  convertita la stringa*/
