...
void input_aggiungi_alimento ( char* nome_file, alimento* alimenti_frigo, unsigned short int numero_alimenti ) {
	alimento alimento_aggiunto; 			  /* alimento che verra'� aggiunto al file */
...
void input_modifica_alimento ( char* nome_file, alimento* alimenti_frigo, unsigned short int numero_alimenti ) {
	...
	alimento alimento_modificato; 			 	 /* variabile che memorizza l'alimento�modificato */
...
unsigned short int is_scaduto (alimento alimento_frigo, data data_corrente) {
	...
	if (alimento_frigo.data_scadenza.anno > data_corrente.anno) {
		return  0;
	} else if (alimento_frigo.data_scadenza.mese > data_corrente.mese && alimento_frigo.data_scadenza.anno == data_corrente.anno) {
		return 0;
	} else if (alimento_frigo.data_scadenza.giorno > data_corrente.giorno && alimento_frigo.data_scadenza.mese == data_corrente.mese){
		return 0;
	} else {		/*se non sono vere tutte queste condizioni la data si scadenza e' = o < alla data corrente*/
		return 1;
	}
}
	...
	for (int i=0; i<numero_alimenti; i++) {
		...
		if (alimenti_frigo[i].id_alimento != 0 && occorrenze_alimento <= soglia_esaurimento ) {			/* se le occorrenze dell'alimento sono <= 2 */
			...
			if (!is_alimento_presente) {		/* se non e' gia' stato inserito, lo inserisce */
				...
				aggiungi_a_lista("lista_spesa.txt", alimenti_frigo[i].nome_alimento);
			}
		}
	}
