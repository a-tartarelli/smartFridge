#include <smartfridge/elaborazione_ricette.h>
#include <smartfridge/gestione_utenti.h>
#include <smartfridge/controlli.h>
#include <smartfridge/carica_file.h>
#include <smartfridge/strutture_dati.h>
 ...
 *@bug se il nome della ricetta che si vuole aggiungere e' gia' presente nel ricettario, essa verra' comunque aggiunta
 */
...
unsigned short int aggiungi_ricetta ( char* nome_file, ricetta ricetta_nuova) {
	...
	if ((file = fopen(nome_file, "rb+")) == NULL) {		/*apertura del file in modalita' scrittura/modifica */
...
void input_modifica_ricetta (char* nome_file, ricetta* ricettario, unsigned short int numero_ricette) {
	...
	ricetta ricetta_modificata; 			  /* alimento che verra'a' modificato file */
	...
	printf ("Quale ricetta vuoi modificare?\n");
	for (int i = 0; i < numero_ricette; i++) {
		if (ricettario[i].id_ricetta > 0) {		/*stampa l'alimento solo se l'id e' valido*/
	...
	while( is_scelta_corretta == 0 ){
		...
		case 5:
			...
			while (!is_compreso_tra_float(0, MAX_CALORIE, ricetta_modificata.kcal)) {
				ricetta_modificata.kcal = inserisci_input_float("Non superare il limite massimo di 2000 kcal, reinserire valore: ", LUNGHEZZA_INPUT);
			}
			is_scelta_corretta = 1;
			break;
...
void input_cancella_ricetta(ricetta* ricettario, unsigned short int numero_ricette) {
	...
	id_ricetta_cancellare = inserisci_input_intero("\n\nInserire l'id della ricetta da cancellare: ", LUNGHEZZA_INPUT);
	...
	if(cancella_ricetta("files/elenco_ricette.bin", id_ricetta_cancellare)) {
		//modifica anche sul vettore di tipo ricetta
		ricettario[id_ricetta_cancellare - 1].id_ricetta = 0;
		printf("\nRicetta cancellata correttamente!\n");
	}
...
unsigned short int input_inserisci_id() {
	...
	printf("\nInserisci ID delle ricetta da cercare: ");
	printf("\n");
