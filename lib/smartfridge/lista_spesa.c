...
unsigned short int stampa_lista_spesa (char* nome_file) {
	...
	if ( (file = fopen (nome_file, "r+")) == NULL ) {
		...
		while ( !feof( file ) ) {		// ciclo che legge i contenuti del file
			...
			if(fgets(record, LUNGHEZZA_NOME, file) != NULL) {
...
unsigned short int aggiungi_a_lista (char nome_file[], char* alimento) {
	...
	if ((file = fopen (nome_file, "r")) == NULL) {
		...
		while (!feof(file)) {		// ciclo che legge i contenuti del file
			...
			fgets(record, LUNGHEZZA_NOME, file);	// effettua una scanf dal file

			if(strncasecmp(record, alimento, strlen(alimento)) == 0) { //controllo se il record letto e' uguale all'alimento passato come parametro
