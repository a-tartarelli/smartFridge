#include <smartfridge/carica_file.h>
...
int main(void) {
    ...
    alimento* alimenti_frigo = carica_elenco_alimenti ( "files/elenco_alimenti.bin", NUMERO_ALIMENTI );	/* vettore di tipo alimento contenente 100 elementi */
    ricetta* ricettario = carica_elenco_ricette ( "files/elenco_ricette.bin", NUMERO_RICETTE );	/* vettore di tipo ricette contenente 50 elementi */
    ricetta* ricettario_copiato = copia_ricettario(ricettario, NUMERO_RICETTE);
    menu_giornaliero* menu_settimanale = carica_menu_settimanale ( "files/menu_settimanale.bin", NUMERO_MENU_GIORNALIERI );	/* vettore di tipo menu_giornaliero contenente 100 elementi */
    ...
    id_utente = seleziona_utente ("files/elenco_utenti.bin");	/*funzione che fa inserire in input un identificativo all'utente e lo assegna a id_utente*/
    ...
    while((scelta = inserisci_input_intero(messaggio, LUNGHEZZA_INPUT))  != NUMERO_VOCI_MENU){
        ...
        switch(scelta){						/* switch che eseguira' un operazione diversa in base all'input dell'utente */
            ...
            case 2:
                id_utente = seleziona_utente ("files/elenco_utenti.bin");
                break;

            case 3:
                id_utente = input_cancella_utente ("files/elenco_utenti.bin", "files/menu_settimanale.bin", id_utente, menu_settimanale, NUMERO_MENU_GIORNALIERI);
                break;
            ...
            case 5:
                input_aggiungi_alimento ("files/elenco_alimenti.bin", alimenti_frigo, NUMERO_ALIMENTI );
                break;

            case 6:
                input_modifica_alimento ("files/elenco_alimenti.bin", alimenti_frigo, NUMERO_ALIMENTI);
                break;
            ...
            case 8:
                input_aggiungi_ricetta ("files/elenco_ricette.bin", ricettario, NUMERO_RICETTE );
                break;

            case 9:
                input_modifica_ricetta ("files/elenco_ricette.bin", ricettario, NUMERO_RICETTE );
                break;
            ...
            case 12:
                input_prepara_ricetta ( "files/menu_settimanale.bin", menu_settimanale, NUMERO_MENU_GIORNALIERI, ricettario, NUMERO_RICETTE, alimenti_frigo, NUMERO_ALIMENTI, id_utente, data_corrente );
                break;

            case 13:
                input_modifica_menu ( "files/menu_settimanale.bin", menu_settimanale, NUMERO_MENU_GIORNALIERI, ricettario, NUMERO_RICETTE, alimenti_frigo, NUMERO_ALIMENTI, id_utente, data_corrente );
                break;

            case 14:
                input_cancella_menu ( "files/menu_settimanale.bin", menu_settimanale, NUMERO_MENU_GIORNALIERI, ricettario, NUMERO_RICETTE, id_utente );
                break;

            case 15:

                if(stampa_lista_spesa("files/lista_spesa.txt") == 0) {
            ...
            case 20:
                if(cancella_lista_spesa("files/lista_spesa.txt")) {
